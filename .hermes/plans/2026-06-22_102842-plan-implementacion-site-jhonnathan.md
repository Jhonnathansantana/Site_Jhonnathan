# Plan de Implementación: Site_Jhonnathan

> **For Hermes:** Use subagent-driven-development skill to implement this plan task-by-task.

**Goal:** Construir un blog personal y profesional moderno, rápido y accesible con Astro, React islands, Tailwind CSS personalizado y Decap CMS; priorizando legibilidad, modo oscuro y buenas prácticas UX/UI.

**Architecture:** Sitio estático generado con Astro. Componentes interactivos livianos con React islands. Contenido editorial gestionado vía Markdown + Decap CMS. Diseño visual propio sobre Tailwind con tipografía Noto Sans, paleta semántica de alto contraste y modo oscuro automático/manual. Búsqueda local con Pagefind en el MVP; funciones de IA pospuestas para fase posterior.
...
- Decap CMS (Git-based)
- Pagefind
- Netlify (hosting recomendado por Identity para Decap CMS)

---

## Decisiones de diseño

- **Tipografía:** Noto Sans para todo el cuerpo. Opcional Noto Sans Display para títulos grandes.
...
- **Técnicas UX/UI:** jerarquía tipográfica clara, espaciado generoso, líneas de lectura controladas, botones grandes, feedback en interacciones, accesibilidad básica (contrastes, foco visible, `aria-label`).
...
- Modify: `astro.config.mjs` (base styles false ya hecho)
...
Download desde Google Fonts Noto Sans regular y bold en formato woff2. Guardar en:
...
Abrir `http://localhost:4321/design-system` y verificar que:
- Noto Sans se aplica.
- Los colores se ven sin fatigar la vista.
...
**Step 4: Instalar @tailwindcss/typography para .prose**
...
Si el usuario no tiene foto aún, usar un SVG placeholder neutro en `public/images/profile.svg`.
...
#### Task 5.1: Panel de administración y configuración
...
- Configurar variable de entorno si aplica.
- Habilitar Netlify Identity.
- Habilitar Git Gateway en Netlify Identity settings.

**Step 3: Probar admin y publicación**

Abrir `https://<site>.netlify.app/admin/`, autenticar con Netlify Identity, crear un post de prueba y verificar que el commit llega a GitHub.
...
4. Verificar que el post aparece publicado en `/blog/`.
...
| API keys de IA expuestas | **No implementar IA en MVP.** Postergar a fase 2 con backend serverless. |
...
## Plan de IA pospuesto (fase 2)

No incluido en este MVP. Cuando se quiera agregar:

1. **Búsqueda semántica:** precomputar embeddings de posts al build time, almacenar en JSON estático, consultar con cosine similarity desde un endpoint serverless.
2. **Asistente de redacción:** crear widget custom de Decap CMS que llame a un endpoint serverless con la API key protegida.

Ambos requieren proteger la API key en Netlify Functions o Vercel Serverless.
